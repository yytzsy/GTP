...
def train(sub_dir, logging, model_save_dir, result_save_dir):
    ...
    model = GTP_C(word_emb_init,is_training = True)
    ...
    for epoch in range(Params.n_epochs):
        ...
        for current_batch_file_idx in xrange(len(train_data)):
            ...
            _, loss_val, choose_loss, regular_loss, probability, G_pre, G, G_softmax = sess.run(
                    [train_op, model.loss, model.choose_loss, model.regular_loss, model.probability, model.G_pre, model.G, model.G_softmax],
                    feed_dict={
            ...
            loss_epoch[current_batch_file_idx] = choose_loss
            logging.info("loss = {:f} choose_loss = {:f} regular_loss = {:f}".format(loss_val,choose_loss,regular_loss))
           

        logging.info("Epoch: {:d} done.".format(epoch))
        tStop_epoch = time.time()
        logging.info('Current Epoch Mean loss forward: {:f}'.format(np.mean(loss_epoch)))
...
def test(model_save_dir, result_save_dir):
    ...
    model = GTP_C(word_emb_init,is_training = False)
    config = tf.ConfigProto(allow_soft_placement=True)
    config.gpu_options.per_process_gpu_memory_fraction = 0.2
    sess = tf.InteractiveSession(config=config)
    with tf.device("/cpu:0"):
    ...
    for epoch in range(all_epoch+1):
        ...
        for current_batch_file_idx in xrange(len(val_data)):
            ...
            batch_probability,G_softmax = sess.run([model.probability,model.G_softmax],
                    feed_dict={
                        model.video: current_video_feats,
                        model.sentence_index: current_caption_matrix,
                        model.video_w_len: current_video_length,
            ...
            for i in range(Params.batch_size):
                ...
                for j in range(Params.max_choose_len):
                    ...
                    idx = idx_list[-1]
                    if idx == Params.max_video_len and len(choose_index) > 0:
                        break
                    elif idx == Params.max_video_len and len(choose_index) == 0:
                        idx_list = idx_list[:-1]
                        idx = idx_list[-1]
                        choose_index.append(idx)
                    else:
                        while idx in choose_index:
                            idx_list = idx_list[:-1]
                            idx = idx_list[-1]
