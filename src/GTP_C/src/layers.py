...
def sequence_pointer_net(video, video_len, sentence_state, cell, params, scope = "sequence_pointer_network"):
    with tf.variable_scope(scope):
        ...
        for i in range(Params.max_choose_len+1):
            with tf.variable_scope('sequence_attention'):
                if i > 0: tf.get_variable_scope().reuse_variables()
                p_logits = attention_with_padding(inputs, Params.attn_size, weights_p, memory_len = video_len, scope = "attention")
                p_logits_list.append(p_logits)
                scores = tf.expand_dims(p_logits, -1)
                attention_pool = tf.reduce_sum(scores * expand_video,1)
                _, state = cell(attention_pool, state)
                inputs = [expand_video, state]
...
def attention_with_padding(inputs, units, weights, scope = "attention", memory_len = None, reuse = None):
    with tf.variable_scope(scope, reuse = reuse):
        outputs_ = []
        weights, v = weights
        for i, (inp,w) in enumerate(zip(inputs,weights)):
        ...
        scores = tf.reduce_sum(tf.tanh(outputs) * v, [-1])
        if memory_len is not None:
            scores = mask_attn_score_with_padding(scores, memory_len)
        return tf.nn.softmax(scores) # all attention output is softmaxed now
