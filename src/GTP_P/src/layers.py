...
def get_attn_params(attn_size,initializer = tf.truncated_normal_initializer):
    ...
    with tf.variable_scope("attention_weights"):
        params = {
                ...
                "W_classify": tf.get_variable("W_classify", dtype = tf.float32, shape = (2*attn_size, 1), initializer = initializer())
                }
        return params
...
def mask_loss(score, memory_sequence_length, score_mask_value = 0.0):
...
def GCN(G,inputs,W_c,scope,is_training):
    ...
    with tf.variable_scope(scope):
        ...
        # Z = BatchNormalization(Z,is_training = is_training,scope = scope)
...
def outputs_probability(video, video_len, sentence_state, cell, params, scope = "outputs_probability"):
    with tf.variable_scope(scope):
        weights_q, weights_p = params
        shapes = video.get_shape().as_list()
        inputs = [video, sentence_state]
        p_logits = attention(inputs, Params.attn_size, weights_p, memory_len = video_len, scope = "attention")
        return p_logits




def outputs_probability_sig(video, video_len, sentence_state, cell, params, scope = "outputs_probability_sig"):
    with tf.variable_scope(scope):
        weights_q, weights_p = params
        shapes = video.get_shape().as_list()
        inputs = [video, sentence_state]
        p_logits = attention_sig(inputs, Params.attn_size, weights_p, memory_len = video_len, scope = "attention")
        return p_logits


def outputs_probability_classify(video,video_len,weights):
    with tf.variable_scope("outputs_probability_classify"):
        shapes = video.get_shape().as_list()
        video_flat = tf.reshape(video,[-1,shapes[-1]])
        p_logits = tf.matmul(video_flat,weights)
        p_logits = tf.reshape(p_logits,[shapes[0],shapes[1]])
        p_logits = mask_attn_score(p_logits, video_len)
        return tf.sigmoid(p_logits)





def attention_sig(inputs, units, weights, scope = "attention", memory_len = None, reuse = None):
    with tf.variable_scope(scope, reuse = reuse):
        outputs_ = []
        weights, v = weights
        ...
        return tf.nn.sigmoid(scores) # all attention output is softmaxed now
...
def avg_sentence_pooling(memory, units, weights, memory_len = None, scope = "sentence_pooling"):
    with tf.variable_scope(scope):
...
def cross_entropy_old(output, target, video_len):
    cross_entropy = target * tf.log(output + 1e-8)
    cross_entropy = mask_loss(cross_entropy,video_len)
    cross_entropy = -tf.reduce_sum(cross_entropy, 1) # sum across video timestep
    cross_entropy = tf.reduce_mean(cross_entropy, 0) # average across batch
    return cross_entropy



def cross_entropy(output, target, video_len):
    cross_entropy = target * tf.log(output + 1e-8) + (1.0 - target) * tf.log( 1.0 - output + 1e-8)
    cross_entropy = mask_loss(cross_entropy,video_len)
    cross_entropy = -tf.reduce_sum(cross_entropy, 1) # sum across video timestep
    cross_entropy = tf.truediv(cross_entropy,tf.cast(video_len,tf.float32))
    cross_entropy = tf.reduce_mean(cross_entropy, 0) # average across batch
    return cross_entropy
